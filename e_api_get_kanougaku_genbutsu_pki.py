...
# 備考:
#
def func_kanougaku_genbutsu(int_p_no, class_login_property):
    # 送信項目の解説は、マニュアル「立花証券・ｅ支店・ＡＰＩ（ｖ〇）、REQUEST I/F、機能毎引数項目仕様」
    # p13/46 No.10 引数名:CLMZanKaiKanougaku を参照してください。

    dic_req_item = {}
    str_p_sd_date = func_p_sd_date(datetime.datetime.now())     # システム時刻を所定の書式で取得

    str_key = 'p_no'
    str_value = str(int_p_no)
    dic_req_item[str_key] = str_value
    ...
    # URL文字列の作成
    str_url = func_make_url_request_from_dic(False, \
